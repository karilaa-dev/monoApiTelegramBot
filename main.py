...
async def send_start(message: types.Message):
    ...
    await message.answer(startMessage, parse_mode="HTML", reply_markup=keyboard)
    ...
    if match("^[A-Za-z0-9_-]*$", message.text) is not None and len(message.text) == 44:
        ...
        if "errorDescription" in tTest:
            if tTest["errorDescription"] in ["Missing one of required headers 'X-Token' or 'X-Key-Id'", "Unknown 'X-Token'"]:
                ...
                await message.answer('Вы успешно изменили токен', reply_markup=keyboardToken)
        else:
            db.update({'api': b64encode(message.text)}, find.id == message.chat.id)
...
async def send_reset(message: types.Message):
    if await check_access(message) is True:
        await message.answer("Вы уверены что ходите сбросить настройки бота?", reply_markup=keyboardDelToken)
        await opt.reset.set()

@dp.message_handler(state=opt.reset)
async def send_reseting(message: types.Message, state: FSMContext):
    if message.text == "Да":
        db.update({'api': None, 'req': None, 'name': None}, find.id == message.chat.id)
        await message.answer('Вы сбросили настройки', reply_markup=keyboard)
        await state.finish()
    elif message.text == "Нет":
        await state.finish()
        await message.answer('Вы вернулись в настройки', reply_markup=keyboardOpt)
    else:
        await opt.reset.set()
